...
class ALP_Attention(nn.Module):
    def __init__(self, channel):
        super(ALP_Attention, self).__init__()

        self.conv1_1 = nn.Conv2d(channel, channel, kernel_size=1)
        self.conv1_2 = nn.Conv2d(channel, channel, kernel_size=(8, 1))
        self.conv1_3 = nn.Conv2d(2 * channel, channel, kernel_size=1)
        self.conv1_4 = nn.Conv2d(channel, channel, kernel_size=1)
    ...
    def alp_attention(self, F_h, F_g):
        x_h = self.conv1_1(F_h)
        x_g = self.conv1_2(F_g)
        x_l = torch.cat((x_h, x_g), dim=1)
        x_l = self.conv1_3(x_l)
        x_l = self.bn(x_l)
        F_l = self.relu(x_l)
        ...
        beta = F_s * F_h
        beta = beta.sum(axis=(2, 3)).unsqueeze(2).unsqueeze(3)
        beta = self.conv1_4(beta)
        beta = self.sigmoid(beta)
        beta_ = beta.repeat(1, 1, list(F_h.shape)[2], list(F_h.shape)[3])

        F_ = beta_ * F_h
        return F_

    def forward(self, F_h, F_v):
        F_ = self.alp_attention(F_h, F_v)
...
class SRCNet(nn.Module):
    def __init__(self, d, A, total_blocks, ch_list):
        ...
        self.attention2d = ALP_Attention(ch_list[3])
    ...
    def forward(self, x):
        ...
        F_ = self.attention2d(F_h, F_g)
        # ===================== Loc-Stage ====================== #

        # ===================== Pred-Stage ===================== #
        out_1 = self.classifier_1(F_).squeeze(2)[:, :, 0]
        out_2 = self.classifier_2(F_).squeeze(2)[:, :, 1]
        out_3 = self.classifier_3(F_).squeeze(2)[:, :, 2:].permute(0, 2, 1)
        # ===================== Pred-Stage ===================== #
